...
function UpdatePurchase() {
  ...
  const [custId, setCustId] = useState(queryParams.get("sup_id"));
  ...
  const handleRowClick = (row) => {
    setSelectedRow(row);
    setSalesProdId(row.pur_prod_id);
    setProdId(row.p_id);
...
// Getting Product details and sale details
    ...
    const fetchData = async () => {
      try {
        const [invoiceRes, invoiceProd] = await Promise.all([
          ...
          axios.get(
            `${GlobalService.path}/fetchPProduct/${date}/${invoice_no}`
          ),
        ]);
        ...
        // Customer Details
        setCustomerName(invoiceRes.data.data.sup_name);
        setContact(invoiceRes.data.data.mobile_no);
  ...
  const addProductDetails = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${GlobalService.path}/updatePurchaseProduct/${sales_prod_id}/${invoice_no}`,
        productDetails
      );
  ...
  const submitSaleProduct = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${GlobalService.path}/updatePurchase/${date}/${invoice_no}`,
        saleProductDetails
      );
  ...
  useEffect(() => {
    ...
    const tweight = Number(weight * quantity)
  ...
  }, [CGST, SGST, IGST, transportAmount, otherAmount, subTotal,quantity]);
